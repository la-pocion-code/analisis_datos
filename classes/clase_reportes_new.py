...
class ReportClassNew():
    ...
    def informe_cartera(self, categorias: list, client_credit_expo: dict) -> pd.DataFrame:
        ...
        def build_email_html_consolidado(df):
            ...
            # Consolidar por TIPO CLIENTE: un tipo puede ser atendido por varios
            # responsables (clientes distintos) y no debe aparecer duplicado aquí.
            df_tipo = (
                df.groupby('TIPO CLIENTE', as_index=False)
                  .agg({
                      'CLIENTE':  'sum',
                      'CORRIENTE':'sum',
                      'PROXIMO':  'sum',
